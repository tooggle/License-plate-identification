...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    # 绘制边界框
    draw.rectangle([c1, (c1[0] + text_width, c1[1] - text_height - 1)], outline=color, width=line_thickness)
    # 绘制文本背景
    draw.rectangle([c1, (c1[0] + text_width, c1[1])], fill=color)
    # 绘制文本
    draw.text((c1[0], c1[1] - text_height - 5), label, font=font, fill=(255, 255, 255))
    # 将PIL图像转换回OpenCV图像
